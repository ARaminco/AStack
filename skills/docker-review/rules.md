...
## معیار کیفیت
- بررسی `multi-stage builds` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `image size` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `non-root users` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `secrets` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `health checks` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `layer caching` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `local parity` و ثبت وضعیت pass، fail یا not applicable.
