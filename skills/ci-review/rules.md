...
## معیار کیفیت
- بررسی `GitHub Actions` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `workflow triggers` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `permissions` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `caching` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `matrix strategy` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `secrets` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `artifacts` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `required checks` و ثبت وضعیت pass، fail یا not applicable.
