...
## معیار کیفیت
- بررسی `model usage` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `token budgets` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `database sizing` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `cache strategy` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `queue workers` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `storage lifecycle` و ثبت وضعیت pass، fail یا not applicable.
- بررسی `billing alerts` و ثبت وضعیت pass، fail یا not applicable.
